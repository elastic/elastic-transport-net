...
public class AddressParsing
{
	...
	public void IsMatched()
	{
		...
		for (var i = 0; i < testcases.GetLength(0); i++)
		{
			...
			_ = match.Groups["ip"].Value.Should().BeEquivalentTo(ip);
			_ = match.Groups["port"].Value.Should().BeEquivalentTo(port);
		}
	}
	...
	public void FqdnIsReadCorrectly()
	{
		...
		for (var i = 0; i < testcases.GetLength(0); i++)
		{
			...
			_ = match.Groups["fqdn"].Value.Should().BeEquivalentTo(fqdn);
			_ = match.Groups["ip"].Value.Should().BeEquivalentTo(ip);
			_ = match.Groups["port"].Value.Should().BeEquivalentTo(port);
		}
	}
