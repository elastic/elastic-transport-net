...
public sealed class DefaultMetaHeaderProducer : MetaHeaderProducer
{
	...
	public override string? ProduceHeaderValue(BoundConfiguration boundConfiguration, bool isAsync)
	{
		try
		{
			...
			if (boundConfiguration.RequestMetaData?.Items is not null && boundConfiguration.RequestMetaData.Items.TryGetValue(RequestMetaData.HelperKey, out var helperSuffix))
